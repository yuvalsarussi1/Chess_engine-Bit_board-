...
def castling_condition_King_side(side) -> bool:
    if side == 0:  
        if b.WHITE_KING_MOVE or b.WHITE_RR_MOVE: return False
        if b.ALL_OCCUPANCY & b.WHITE_KINGSIDE_EMPTY: return False
        if th.square_attacked_by_black(4): return False  
        if th.square_attacked_by_black(5): return False  
        if th.square_attacked_by_black(6): return False  
        ...
        if th.square_attacked_by_white(60): return False  
        if th.square_attacked_by_white(61): return False  
        if th.square_attacked_by_white(62): return False  
...
def castling_condition_Queen_side(side) ->bool:
    if side == 0:  
        ...
        if th.square_attacked_by_black(4): return False  
        if th.square_attacked_by_black(3): return False  
        if th.square_attacked_by_black(2): return False  
        ...
        if th.square_attacked_by_white(60): return False  
        if th.square_attacked_by_white(59): return False  
        if th.square_attacked_by_white(58): return False  
